...
SmartPerfetto 使用 YAML 驱动的 Skill 系统，支持可扩展的分析规则和厂商定制：
...
│                       SkillExecutor                                  │
├─────────────────────────────────────────────────────────────────────┤
│  1. loadSkill(skillId, vendor?)  - 加载 YAML 定义                   │
│  2. substituteVariables()         - 变量替换 (${package}, ${item})  │
│  3. executeStep()                 - 执行 SQL 步骤                    │
│  4. evaluateDiagnostics()         - 评估诊断规则                     │
│  5. generateSummary()             - 生成分析摘要                     │
└──────────────┬──────────────────────────────────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────────────────────────────────┐
...
│  skills/                                                             │
│  ├── base/                        # 8 个基础 Skills                  │
│  │   ├── startup.skill.yaml       # 启动分析                        │
│  │   ├── scrolling.skill.yaml     # 滑动分析 (v3.0 分层递进式)       │
│  │   ├── memory.skill.yaml        # 内存分析                        │
│  │   ├── cpu.skill.yaml           # CPU 分析                        │
│  │   ├── binder.skill.yaml        # Binder IPC 分析                 │
│  │   ├── surfaceflinger.skill.yaml# SF 分析                         │
...
│   │   │   └── skillEngine/                    # Skill Engine (YAML 驱动)
│   │   │       ├── skillLoader.ts              # Skill 加载器
│   │   │       ├── skillExecutor.ts            # Skill 执行器
│   │   │       ├── skillAnalysisAdapter.ts     # API 适配器
│   │   │       ├── types.ts                    # 类型定义
│   │   │       └── index.ts                    # 导出
│   │   ├── cli/                                # CLI 工具
│   │   │   ├── index.ts                        # CLI 入口
...
│   │   ├── base/                               # 基础 Skills (8 个)
│   │   │   ├── startup.skill.yaml
│   │   │   ├── scrolling.skill.yaml
│   │   │   ├── memory.skill.yaml
│   │   │   ├── cpu.skill.yaml
│   │   │   ├── binder.skill.yaml
│   │   │   ├── surfaceflinger.skill.yaml
│   │   │   ├── navigation.skill.yaml
│   │   │   └── click_response.skill.yaml
│   │   ├── vendors/                            # 厂商定制
│   │   │   ├── oppo/, vivo/, xiaomi/...
│   │   │   └── *.override.yaml
│   │   ├── custom/                             # 用户自定义
